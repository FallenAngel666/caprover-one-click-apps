...
const pathOfStatic = path.join(__dirname, '..', `static`);

const pathOfDist = path.join(__dirname, '..', `dist`);
const pathOfStaticDistContent = path.join(__dirname, '..', `dist`, 'static');

const pathOfDistV2 = path.join(pathOfStaticDistContent, 'v2');
const pathOfDistV3 = path.join(pathOfStaticDistContent, 'v3');
const pathOfDistV4 = path.join(pathOfStaticDistContent, 'v4');
...
function buildDist() {
    return fs.readdir(pathOfSourceDirectoryApps)
        ...
        .then(function () {
            fs.copySync(path.join(pathOfStatic, 'Dockerfile'), path.join(pathOfDist, 'Dockerfile'));
            fs.copySync(path.join(pathOfStatic, 'index.html'), path.join(pathOfStaticDistContent, 'index.html'));
            fs.copySync(path.join(pathOfStatic, 'captain-definition'), path.join(pathOfDist, 'captain-definition'));
            return fs.copySync(path.join(pathOfPublic, 'CNAME'), path.join(pathOfStaticDistContent, 'CNAME'));
        });
}
